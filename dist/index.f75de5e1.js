...
function sortTable(columnIndex, dataType = "text") {
    ...
    rows.sort((a, b)=>{
        const cellA = a.querySelector(`td:nth-child(${columnIndex})`).textContent.trim();
        const cellB = b.querySelector(`td:nth-child(${columnIndex})`).textContent.trim();
        if (dataType === "number") return parseSalary(cellA) - parseSalary(cellB); // Numeric sorting
